  ...
  const feedItems = [
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1509391366360-2e959784a276?w=500&auto=format&fit=crop&q=80",
    ...
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1618477388954-7852f32655ec?w=500&auto=format&fit=crop&q=80",
    ...
    <div className="space-y-6 max-w-5xl mx-auto p-4 md:p-6 bg-[#0B120F] text-slate-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Connect with collective eco-guardians, stream real-time verifications, and observe macro-environmental telemetry.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#111A16] border border-[#14231C] p-4 rounded-xl">
          <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-1">CO2 Offset Collectively</div>
          <div className="text-xl font-black text-white tracking-tight">142,804.5 <span className="text-xs text-[#10B981] font-normal">Tons</span></div>
          <div className="text-[9px] text-slate-400 font-mono mt-1 flex items-center gap-1">
            <TrendingUp size={10} className="text-[#10B981]" /> +3.4% vs last week
          </div>
        </div>
        <div className="bg-[#111A16] border border-[#14231C] p-4 rounded-xl">
          <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-1">Total Verified Actions</div>
          <div className="text-xl font-black text-white tracking-tight">892,412</div>
          <div className="text-[9px] text-[#10B981] font-mono mt-1 flex items-center gap-1">
            <ShieldCheck size={10} /> 99.8% Protocol Authenticated
          </div>
        </div>
        <div className="bg-[#111A16] border border-[#14231C] p-4 rounded-xl">
          <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-1">Active Eco-Guardians</div>
          <div className="text-xl font-black text-white tracking-tight">41,205 <span className="text-xs text-slate-500 font-normal">Online</span></div>
          <div className="text-[9px] text-slate-400 font-mono mt-1 flex items-center gap-1">
            <span className="w-1.5 h-1.5 bg-[#10B981] rounded-full inline-block animate-ping" /> Node updates synchronized
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {feedItems.map((item) => (
            <div key={item.id} className="bg-[#111A16] border border-[#14231C] rounded-xl overflow-hidden p-4 space-y-3">
              
              {/* User Profile Info Header */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <img src={item.avatar} alt={item.user} className="w-8 h-8 rounded-lg object-cover border border-[#14231C]" />
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-white text-xs leading-none">{item.user}</span>
                ...
                <div className="text-right">
                  <span className="text-xs font-mono font-black text-[#10B981] block">+{item.points} PTS</span>
                  <span className="text-[9px] font-mono text-slate-500 uppercase">Verified Yield</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6 pt-1 text-slate-400 text-xs font-mono">
                ...
                >
                  <Heart size={14} fill={likedFeeds[item.id] ? "currentColor" : "transparent"} />
                  <span>{likedFeeds[item.id] ? item.likes + 1 : item.likes}</span>
                </button>
          ...
          <div className="bg-[#111A16] border border-[#14231C] p-4 rounded-xl space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-mono">
                <span className="text-slate-400">Mangrove Forestation target</span>
                <span className="text-[#10B981] font-bold">84%</span>
              </div>
              <div className="w-full bg-[#1A2E24] rounded-full h-2">
                <div className="bg-[#10B981] h-2 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.4)]" style={{ width: '84%' }}></div>
              </div>
