...
export default function Navbar() {
  ...
  // Helper function to check if link is currently active
  const checkIsActive = (path) => {
    if (path.startsWith("#")) {
      return location.hash === path;
    }
    // Match exact path when hash is empty
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          {navLinks.map((link) => {
            ...
            return isAnchor ? (
              <a
                key={link.name}
                href={link.path}
                className={`relative px-4 py-1.5 text-xs font-semibold tracking-wide rounded-full transition-all duration-200 ${
                  isActive
        ...
        <button
          ...
          className="md:hidden text-gray-300 hover:text-emerald-400 p-2 focus:outline-none"
