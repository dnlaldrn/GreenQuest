...
export default function VideoReviewTab({ statusFilter = "all" }) {
  ...
  const handleOpenVideo = async (submission) => {
    ...
    try {
      ...
      console.error(
        "Failed to get signed URL, falling back to stored video_url:",
        err,
      );
      ...
      <div className="p-6 border-b border-[#DCE5D9]/10 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-[#DCE5D9]">
            Eco Video Verification Queue
          </h3>
          <p className="text-xs text-[#BCCBB9]">
            Review pending video evidence where AI checks flagged confidence
            criteria.
          </p>
        </div>
        <button
          ...
          <tbody className="divide-y divide-[#DCE5D9]/5">
            {loading ? (
              <tr>
                <td
                  colSpan="6"
                  className="px-6 py-8 text-center text-xs text-[#BCCBB9] font-mono"
                >
              ...
              submissions.map((s) => {
                ...
                return (
                  <tr
                    key={s.id}
                    className="hover:bg-[#333B33]/10 transition-colors"
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[#DCE5D9]">
                            {profile?.username ?? "Eco Participant"}
                          </p>
                          <span className="text-[10px] text-[#BCCBB9] font-mono">
                            Points: {s.status === "approved" ? "+5" : "0"}{" "}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 max-w-xs">
                      <p className="text-xs text-[#DCE5D9] line-clamp-2">
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center group-hover:bg-black/20 transition-all">
                          <Play
                            size={16}
                            className="text-white fill-white shadow-lg scale-100 group-hover:scale-110 transition-transform"
                          />
                        </div>
                    ...
                    <td className="px-6 py-4">
                      ...
                      {s.ai_feedback && (
                        <span
                          className="text-[9px] text-[#BCCBB9] block mt-0.5 line-clamp-1 truncate max-w-[150px]"
                          title={s.ai_feedback}
                        >
                          {s.ai_feedback}
                        </span>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        <span
                          className={`w-1 h-1 rounded-full ${s.status === "approved" ? "bg-[#4BE277]" : "bg-[#FFB4AB]"}`}
                        ></span>
            ...
            <div className="bg-[#161D16] border border-[#4BE277]/30 shadow-[0_0_40px_rgba(74,225,118,0.15)] max-w-2xl w-full rounded-2xl overflow-hidden p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-black/30 border border-[#3D4A3D]/50 p-3 rounded-xl font-mono text-[11px] text-[#BCCBB9]">
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-[#92DB2A]">+5 pts</p>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-[#DCE5D9]/10">
                <div className="text-[9px] text-[#BCCBB9] font-mono">
                  Status:{" "}
                  <span className="uppercase text-[#FFB4AB]">
